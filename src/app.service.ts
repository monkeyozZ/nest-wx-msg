...
export class AppService {
  ...
  async send() {
    ...
    console.log('获取token 结果: %j', token, data);
    // 模板消息接口文档
    const users = config.weChat.users;
    const promise = users.map(id => {
      console.log('--------------开始发送每日提醒-----------------------------------------------: %j', id);
      data['touser'] = id;
      return this.toWechart(token, data);
    });
    const results = await Promise.all(promise);
    console.log('--------------结束发送每日提醒->结果-----------------------------------------------: %j', results);
    return results;
  }
  async getOpenId() {
    const url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' + config.weChat.appId + '&secret=' + config.weChat.secret + '&code=021nM00w3dCJlX2ULo2w3A2ayy1nM00A&grant_type=authorization_code';
  ...
  async toWechart(token, data) {
    ...
    const result = await this.httpService.post(url, data).pipe(map(res => res.data)).toPromise();
    return result;
  }
  // 获取token
  async getToken() {
  ...
  async getTemplateData() {
    ...
    // 发工资模板
    if (!wageDay) {
      data['template_id'] = config.weChat.wageDay;
      data.data = {
        dateTime: {
      ...
      // 结婚纪念日模板
      data['template_id'] = config.weChat.marry;
      data.data = {
        dateTime: {
          value: this.getDatetime(),
          color: '#cc33cc',
      ...
      // 生日模板
      data['template_id'] = config.weChat.birthday;
      data['url'] = config.weChat.birthdayUrl;
      data.data = {
        dateTime: {
          value: this.getDatetime(),
          color: '#cc33cc',
      ...
      // 正常模板
      data['template_id'] = config.weChat.daily;
      // 获取天气
      ...
      data.data = {
        ...
        tem1: {
          value: getWeather.tem_day,
          color: '#ff0000',
        },
        tem2: {
          value: getWeather.tem_night,
          color: '#33ff33',
        },
        win: {
  ...
  // 获取天气
  async getWeather(city = '上海') {
    const url = 'https://www.tianqiapi.com/free/day?unescape=1&appid=' + config.weather.appid + '&appsecret=' + config.weather.appsecret + '&city=' + encodeURI(city);
    const result = await this.httpService.get(url).toPromise();
  ...
  // 获取 下次发工资 还有多少天
  /* getWageDay() {
    const wage = config.time.wageDay;
    // 获取日期 day
  ...
  // 获取 距离 下次生日还有多少天
  getbirthday() {
    const birthday = config.time.birthday[moment().year()];
    // 获取当前时间戳
    const now = moment(moment().format('YYYY-MM-DD')).valueOf();
    // 获取纪念日 月-日
    ...
    if (now > nowTimeNumber) {
      // 获取明年目标日期
      resultBirthday = moment(config.time.birthday[y + 1]).valueOf();
    }
    return moment(moment(resultBirthday).format()).diff(moment(now).format(), 'day');
  }
  // 获取 相恋天数
  getLoveDay() {
    const loveDay = config.time.love;
